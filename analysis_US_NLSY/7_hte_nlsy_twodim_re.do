...
est store men_rel_II
	...
	* Marginal Effects Plot for Model II
	...
	marginsplot, ///
		title("") yscale(range(-0.4(0.2)1)) ylabel(-0.4(0.2)1) ///
		ytitle("Return to College") xtitle("Propensity of College (School-relevant)") 

		graph export "$figures/nlsy_hte_margins_rel_men_resid.pdf", replace

* Model III 
xtmixed lnwg c.ps_men_rel##college##c.age_c || id: , mle variance pweight(ipw)
est store men_rel_III
...
est store women_rel_II
	...
	* Marginal Effects Plot for Model II
	...
	marginsplot, ///
		...
		graph export "$figures/nlsy_hte_margins_rel_women_resid.pdf", replace

* Model III
xtmixed lnwg c.ps_women_rel##college##c.age_c || id: , mle variance pweight(ipw)
est store women_rel_III 
...
est store men_irr_II
	...
	* Marginal Effects Plot for Model II 
	...
	marginsplot, ///
		...
		graph export "$figures/nlsy_hte_margins_irr_men.pdf", replace
...
est store women_irr_II
	...
	* Marginal Effects Plot for Model II
	...
	marginsplot, ///
		...
		graph export "$figures/nlsy_hte_margins_irr_women.pdf", replace
...
esttab men_rel_I men_rel_II women_rel_I women_rel_II ///
	using "$tables/nlsy_hte_rel_re_resid.rtf" ///
	, nobaselevels label nodepvars se r2 b(%8.2f) replace 

esttab men_irr_I men_irr_II women_irr_I women_irr_II ///
	using "$tables/nlsy_hte_irr_re.rtf" ///
	, nobaselevels label nodepvars se r2 b(%8.2f) transform(ln*: exp(2*@) 2*exp(2*@)) replace  
...
* Appendix Table
esttab men_rel_III men_irr_III women_rel_III women_irr_III ///
	using "$tables/nlsy_hte_twodim_agetrends_resid.rtf" ///
...
combomarginsplot ///
	...
	yscale(range(-0.4(0.2)1.1)) ylabel(-0.4(0.2)1.1) legend(pos(1) col(2)) ///
	aspectratio(1) ///
...
graph save twodim_men.gph, replace

graph export "$figures/nlsy_hte_margins_twodim_men_resid.pdf", replace
...
graph save twodim_women.gph, replace

graph export "$figures/nlsy_hte_margins_twodim_women_resid.pdf", replace
